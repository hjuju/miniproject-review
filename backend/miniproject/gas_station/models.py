...
from project.common.models import Reader, Printer, Scraper
import numpy as np
import pandas as pd
import folium
...
class Service():
    ...
    def gas_station_price_info(self):
        ...
        reader = self.reader
        printer = self.printer
        station_files = glob('./data/지역_위치별*xls')  # *: xls의 모든 파일을 가져와라
        tmp_raw = []
        for i in station_files:
            t = pd.read_excel(i, header=2)
            tmp_raw.append(t)
        station_raw = pd.concat(tmp_raw)  # concat -> 리스트끼리 합침
        station_raw.info()
        '''
        print("*"*100)
