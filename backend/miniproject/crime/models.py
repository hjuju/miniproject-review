...
import pandas as pd
import numpy as np
from project.common.models import FileDTO
from project.common.models import Reader, Printer
from sklearn import preprocessing
import folium

'''
* 문제정의 *
서울시의 범죄현황과 CCTV 현황을 분석해서 정해진 예산 안에서
구별로 다음해에 배분하는 기준을 마련
예산금액을 입력하면, 구당 할당되는 CCTV 카운터를 자동으로 알려주는 AI 프로그램 작성 
'''
...
class Service(Reader):
    ...
    def save_police_pos(self):
        ...
        crime.loc[crime['관서명'] == '혜와서', ['구별']] == '종로구'
    ...
    def save_cctv_pop(self):
        ...
        # 1이면 세로로, 0은 가로로 지움, inplace는 지운거 복구X
        cctv_pop = pd.merge(cctv, pop, on='구별')  # 같은 구끼리 병합
        cor1 = np.corrcoef(cctv_pop['고령자비율'], cctv_pop['소계'])  # 상관관계 출력
    ...
    def save_folium_map(self):  # 지도로 시각화
        f = self.f
        r = self.r
        p = self.p
        f.context = './data/'
        ...
        f.fname = 'kr_states'
        kr_states = r.json(f)
        f.context = './saved_data/'  # file.context의 경로는 새로운 경로 입력 전까지 이어짐
        f.fname = 'police_norm'
        police_norm = r.csv(f)
        f.fname = 'police_pos'
        police_pos = r.csv(f)
        ...
        temp = police_pos[self.arrest_columns] / police_pos[self.arrest_columns].max()  # .max() 가장 높은 수로 검거율 산정
        ...
        folium.Choropleth(
            geo_data=kr_states,
            name="choropleth",
            data=tuple(zip(police_norm['구별'], police_norm['범죄'])),  # DF는 variable 상태이기 때문에 constant로 변환해야함
        ...
        for i in police_pos.index:
            folium.CircleMarker([police_pos['lat'][i], police_pos['lng'][i]],
                                radius=police_pos['검거'][i] * 10,  # 원 크기
